...
> **Última revisão:** 2026-05-07 — master @ tag v0.2.1.4 (web/docs only — Android Play Store mantém vc 48 v0.2.1.2 até próxima release code) publicada 2026-05-07. **v0.2.1.4 = release docs-only** — refactor completo §6 ROADMAP em 4 categorias visuais (🚀 IMPLEMENTAÇÃO · ✨ MELHORIAS · 🐛 BUGS · 🔄 TURNAROUND) com bolinhas P0/P1/P2/P3 + legenda visual global topo doc + items fechados redistribuídos inline na posição correta. Plus 27 NOVOS items planejamento criados (#162-#189): #162 TreatmentForm UX warning Mounjaro repro; #163-#168 plano egress otimização escala (RPC consolidado + Realtime broadcast + Delta sync + persist + MessagePack + CDN cache); #169-#173 marketing/ASO/growth playbook BR; #174-#187 features differentiators concorrentes (OCR med scan + receita scan + adesão report PDF + WhatsApp share + Modo Alzheimer + Wear OS + health metrics + voz/TTS + refill affiliate + symptom diary + telemedicina + B2B caregiver + Apple Health + Memed/Nexodata); **#188 KILLER feature Mini IA Chat NLP cadastro natural via Anthropic Claude API**; #189 UpdateBanner versionName fix. Pulou versão v0.2.1.3 (reservada pra próxima release code work). Sem AAB build (docs-only — versionCode bumpado pra futuro mas Console mantém vc 48 anterior). Closed Testing #130 aguarda Google re-review pós #158 fixes v0.2.1.2 (~7d).
