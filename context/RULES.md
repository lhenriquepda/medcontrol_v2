...
| **17** | **Validação UI em emulador = Appium + UiAutomator2 SEMPRE.** Evitar CDP direto na porta 9222 (que ignora integração nativa). Em apps híbridos Capacitor, é permitido alternar o contexto do Appium para `WEBVIEW` e injetar cliques ou textos via `driver.execute` para interagir com elementos React, desde que orquestrado pelo ciclo do Appium. | O CDP direto falha silenciosamente com integrações nativas (Capacitor handlers, FCM, background worker). Usar a alternância de contexto híbrida sob o Appium garante precisão no DOM aliada ao controle do SO (ADB/svc/WorkManager). |
