...
> **Última revisão:** 2026-05-25 — **release/v0.2.8.1 EM CURSO** (vc 103, correções de QA, testes unitários e linter da Fase 1 + Grant SELECT em `medications_catalog`). Vitest config atualizado para ignorar pasta `e2e/**`. Ajustes em `dateUtils.test.js` e `statusUtils.test.js` para corrigir asserções quebradas. Correção lógica no offline drain (`markDose.js` `_runDrain`) para reverter o estado local e lançar aviso de erro no Supabase em falhas lógicas (401, 403, 404). Remoção do warn de set-state-in-effect no formulário `TreatmentForm.jsx` (durationUnit atualizado de forma síncrona nos handlers) e de chamada redundante de `setPermState` nas notificações. SQL migration para conceder SELECT na tabela `medications_catalog` eliminando erro HTTP 403 silencioso no autocomplete. **Anterior:** v0.2.8.0 SHIPPED (vc 102, MutationDrainWorker nativo Android). Fila offline migrada para SharedPreferences (`CapacitorStorage` `dosy_pending_mutations`). WorkManager a cada 15min chama `MutationDrainWorker.java` para sincronizar mesmo com app/WebView suspensos. Refresh token nativo em Java. Idempotência server-side com PK `request_id` no `mutation_log`. **Anterior:** release/v0.2.3.17 EM CURSO (vc 80, refactor Fase 2 thread-safety + Fase 4 componentes core). AlarmService static `activePlayer`/`activeVibrator` agora atrás de LOCK + synchronized blocks (mata race "static race em multi-alarm" documentada em PROJETO.md). startMediaPlayerLoop prepara player fora do lock + atomic swap dentro pra reduzir contention. Plus criados 3 componentes core em `src/components/dosy/`: EmptyState (4 variantes), DateRangeChips (scrollable horizontal), StatGrid (2-col MiniStat). Componentes prontos pra adoption gradual nas próximas releases. Plus remove `src/components/BottomSheet.jsx` legacy (0 imports). **Anteriores:** release/v0.2.3.16 EM CURSO (vc 79, refactor Fase 2 partial + Fase 5.8). Fase 2: AlarmActionReceiver.ACTION_ACK chama RPC `confirm_dose` server-side ANTES de abrir o app (mata bug P1 "Ciente não confirma dose"); ACTION_SNOOZE chama RPC nova `snooze_dose` que UPDATE `doses.snoozed_until` (mata bug P1 "Snooze não persiste DB"). Padrão goAsync + Thread + HTTP timeout 1.5s reusando creds `dosy_sync_credentials`. Migration `20260520120000_snooze_dose_rpc_v0_2_3_16` aplicada em prod via apply_migration. Fase 5.8 (Dashboard query optimization): janela default `applyDefaultRange` reduzida de -30d/+60d (90 dias, ~2k doses em conta real) para -7d/+14d (~120 doses) em `dashboardService.js` + `dosesService.js`; `isStaleSync` agora usa `sessionMountedAt` via `useState` lazy initializer pra não disparar banner "Sincronizando dados..." em primeira reabertura quando `dataUpdatedAt` vem hidratado do PersistQueryClient. **Anterior:** release/v0.2.3.15 EM CURSO (vc 78, refactor Fase 1). Entrega: **RealtimeGate** per-queryKey TTL 2.5s (`src/state/realtimeGate.js`) — mutations marcam `markInFlight` em onMutate; useRealtime descarta payloads de keys in-flight em `debouncedInvalidate`. **Versioned cache** (`src/state/versionedCache.js`) — `patchDoseInCache` stampa `_localActedAt: Date.now()` em doses patchadas; reconciler aceita incoming só se `_serverConfirmedAt > _localActedAt + 2500ms`. **Debounces alinhados:** Realtime 1000→2500ms; mutation refetchDoses 2000→1500ms (mutation sempre vence). 4 mutations healthcare (confirmDose/skipDose/undoDose/registerSos) com `markDosesInFlight()` em onMutate + `clearDosesInFlight()` em onSettled. **MultiDoseModal** com `pendingDoseId` per-dose elimina disabled coletivo que travava fila inteira. **Dashboard.handleRefresh** aguarda mutations doses drenarem (≤2s) antes de pull-to-refresh. Mata RC-1 "status volta do nada" and "botão preso" reportados in v0.2.3.14 Internal Testing. Plus: `Refactor_Full.md` (raiz) — plano completo de 5 fases (16 sem.) com §11 inventário de 10 componentes unificáveis. Validado em emulador 5554 (teste-plus) + 5556 (teste-free) — 3 doses marcadas em sequência sem flicker, status persiste 25s, cross-device sync OK. Build OK (vite 22s, gradle 41s, APK 45MB). Aguarda Passo 10.5 STOP pré-AAB. **Anterior:** master @ v0.2.3.14 SHIPPED (vc 77, Play Console Internal Testing 10:45 BRT, tag `v0.2.3.14`). Entrega: #0010 update banner mostra version_name correto (DB autoritativa primeiro, Play Core só se shape semver real) + #0011 modal mandatory robusto a race Play Core `currentVersionCode=null` (query upper-bound-only) + #0009 `usePatientShares` 401 JWT expiry com error state + retry button no SharePatientSheet (em vez de "Carregando..." infinito). Empilhamento C: Sentry breadcrumbs em useAppUpdate fetchReleaseFromDb (debug futuro race Play Core), copy fallback sanitization (sem `v` duplicado, sem chip ugly), debug toggle `__dosyForceFallback`. Validação §11a web Chrome MCP 5/5 OK, §11b emulator SKIP justificado (plugin Play Core depende AAB published), device físico S25 Ultra pendente pós-publish. **Antes:** master @ v0.2.3.13 SHIPPED (vc 76, Play Console Internal Testing 18:04 BRT, tag `3e27811`) — Plano A primeira etapa anti-dose-duplicada (AlarmReceiver pre-check HTTP + cache SharedPrefs + disclaimer paciente compartilhado offline). 5/6 cenários E2E validados emulator+Appium. Bugs #0010+#0011 P2 descobertos e atacados agora em v0.2.3.14. Estado atual → [`context/STATE.md`](STATE.md). **Antes disso:** master @ v0.2.3.12 SHIPPED (vc 75, Play Console Internal Testing 15:19 BRT, tag `c9282e6`). Entrega: #250 ANVISA autocomplete + 9 P1/P2 bug-fixes (#253b email wordmark, #254b self-patient signup, #255 idle skeleton token expirado, #256b SOS submit window.confirm, #257b lockAcquireTimeout, #258b sharing Dashboard RPC, #264 create_treatment dose passada, #265 count exato, #266 PatientDetail mostrar treatment, #267 Dashboard skeleton hour boundary placeholderData cross-key). QA completo Chrome MCP localhost teste-plus@teste.com. 5 bugs P2-P4 abertos próxima release (#259 Cancelada Reports, #260 console errors, #261 SOS en-US format, #262 ad acima header, #263 1 dias termina hoje). vc 69 / 0.2.3.6. **Master @ `v0.2.3.5` SHIPPED** 2026-05-15 (vc 68, tag `bf447d3`, Vercel prod dosymed.app) — UI/UX redesign 5 telas + sistema gradiente unificado + signup branded + #251 share Plus gating fix. **Antes:** master @ `v0.2.3.2` SHIPPED (vc 65, Play Console Internal Testing 14:46 BRT, tag `e277aa6`, Vercel auto-deploy prod dosymed.app). **v0.2.3.2 entregou:** 4 bug-fixes P1/P2 (#227 RLS audit log multi-policy fix, #228 unsubscribeFcm cross-device, #229 AlarmScheduler.commit() sync, #230 Edge v21 group siblings query) + **CLI gradlew destravado** (root cause descoberto: filter driver bloqueia AF_UNIX em `AppData\Local\Temp` — fix `TEMP/TMP` redirect `C:\temp\gradle_tmp` + JDK 25 Adoptium Temurin → AAB CLI 33s autônomo). Validar.md 62 [x] / 0 pending. Counter 142+4=146 fechados. **Antes:** `release/v0.2.3.0` em curso (rebranded `v0.2.3.1`, vc 64). **Refactor Plano A + Fixes B/C aplicado em 7 blocos** após 4 auditorias linha-por-linha revelando 4 root causes arquiteturais (RC-1 dual tray race; RC-2 prefs fire time; RC-3 cancel group hash multi-dose; RC-4 5 paths sem coordenação) não cobertos por v0.2.3.0. 8 commits + bump vc 63→64. **Plano A:** trays foreground migradas Capacitor LocalNotifications → Java AlarmScheduler.scheduleTrayGroup (M2 unificado). **Fix B:** AlarmReceiver consulta SharedPrefs no fire time → re-rota dinâmica. **Fix C:** DosyMessagingService reconstroi `sortedDoseIds.join('|')` hash multi-dose. Edge dose-trigger-handler v20 BATCH_UPDATE/BATCH_DELETE handlers deployed. 3 migrations DB applied. 23 itens código morto removidos. AAB vc 64 + validação device 5 FLUXOS Validar.md pendentes. Master @ **v0.2.2.4** FECHADA. Histórico abaixo continua relevante.. Master @ **v0.2.2.4** FECHADA. Histórico abaixo continua relevante.

> **Refactor v0.2.3.1 detalhe (anterior v0.2.3.0):** 4 commits: bump vc 62→63 + helper unificado JS+Java + scheduler.js + backend Edge functions + Java helper paridade + cleanup. Edge `dose-trigger-handler` v18 + `daily-alarm-sync` v3 deployed via MCP. Migration `expand_dose_change_notify_to_delete_v0_2_3_0` applied. Items inclusos: #217 #220 #221 P1 BUGS + #222 #223 #224 #225 P2/P3 MELHORIAS — todos código mergeado aguardando validação device. Decisões user consolidadas: margem boot 2h, push DnD vibração leve 200ms, cuidador sempre alarme cheio + respeita DnD próprio, janela dinâmica 24h/48h baseada em projeção 400 itens, alarm_audit_log enriquecido `{branch, horizon, source_scenario}` mantém admin.dosymed.app `/alarm-audit` funcional. Master @ **v0.2.2.4** FECHADA (vc 62 Internal Testing 2026-05-13 16:48 BRT, tag `v0.2.2.4`, validado Dosy-Dev + AAB Play Console publicado). **v0.2.2.4 entregou:** **#214 P2 CLEANUP:** Remove `dose_alarms_scheduled` tabela órfã (consumers removidos em #209). 3 mudanças: scheduler.js remove upsert + DosyMessagingService.java remove reportAlarmScheduled + Migration DROP TABLE. Economia ~5-10 MB/dia/device egress. Master @ **v0.2.2.3** FECHADA (vc 61, tag `v0.2.2.3`, AAB Internal Testing pendente). **v0.2.2.3 entregou:** **#213 P1 STORM REAL ROOT CAUSE:** Auditoria logcat Dosy-Dev (~6min) confirmou storm 60s exato vinha de `Dashboard.jsx:99 setInterval setTick(60s)` flipando `todayDoses` ref → useEffect Dashboard:222 chamando `scheduleDoses` redundante. App.jsx top-level signature guard v0.2.2.2 funcionando, mas Dashboard caller sem guard mantinha 1440 reschedules/dia. Fix mínimo: remove caller completo (vestígio pré-#198). Master @ **v0.2.2.2** FECHADA (vc 60 Internal Testing 2026-05-13 15:14 BRT, tag `v0.2.2.2`). **v0.2.2.2 entregou:** **#212 P1 STORM ROOT CAUSE:** Throttle v0.2.2.1 reduziu impacto mas root cause continuou (1.36 batches/min via audit polling 11min). 2 fixes: (a) `useRealtime.js WATCHDOG_INTERVAL_MS` 60s → 300s (5min); (b) `App.jsx useEffect` signature guard via `useMemo dosesSignature`. Esperado: ~10 rescheduleAll/dia (era ~2000, ~30-40 MB/dia egress/device). **Master @ v0.2.2.1** FECHADA (vc 59 Internal Testing 2026-05-13 13:53 BRT, tag `v0.2.2.1`). **v0.2.2.1 entregou:** **#211 P1 HOTFIX** Storm rescheduleAll 1×/min descoberto via audit v0.2.2.0 imediato pós-deploy: (a) `SCHEDULE_WINDOW_MS` 168h→48h alinha plan #209 (era hardcoded 168h mas comentário inline dizia 48h); (b) Throttle module-level rescheduleAll 30s com trailing run pattern — única execução em janela 30s, requests adicionais agendam single trailing run no fim da janela com last args; (c) Audit batch single insert via accumulator (1 insert/batch contendo batch_start + N scheduled + batch_end, antes era 10-100 inserts separados/batch); (d) GRANTS DB service_role + authenticated em alarm_audit_log/config (bug descoberto: tabelas criadas com RLS mas sem GRANTs table-level → Edge Functions e user client silenciava INSERTs). 868 storm rows passados limpos via DELETE direto. **v0.2.2.0 entregou:** **#210 NOVO P1** Sistema auditoria alarmes admin.dosymed.app: tabela `alarm_audit_log` + config whitelist `alarm_audit_config` (seed: lhenrique.pda@gmail.com); captura 6 caminhos (JS scheduler + Java AlarmScheduler/Worker/FCM + Edge daily-sync/trigger-handler); admin pages `/alarm-audit` (filtros + modal) + `/alarm-audit-config` (toggle por email); cron cleanup >7d. **v0.2.1.9 também fechada** (vc 57 publicado 10:09 BRT, substituído por vc 58 mesmo dia): **#209 NOVO P0** Refactor sistema alarmes + push pós 3 bugs reportados: (1) alarme "Sem Paciente" — `DoseSyncWorker` JOIN patients fix; (2) push 5am dose 8am — RPC `update_treatment_schedule` `AT TIME ZONE` correction + data-fix idempotente; (3) cascata Bug 2 — substituído sistema 5 caminhos por arquitetura unificada: `daily-alarm-sync` cron 5am BRT 48h horizon + `dose-trigger-handler` v16 com action `cancel_alarms` (DELETE/UPDATE status-change) + `AlarmScheduler.cancelAlarm` + `DosyMessagingService.handleCancelAlarms` + UNSCHEDULE crons antigos. Egress -99%. Plus fix #208 (VERSION_CODE_TO_NAME map). AAB vc 57 + Internal Testing pendente. Master @ **v0.2.1.8** FECHADA (vc 56 Internal Testing 2026-05-11 22:45 BRT, Vercel prod 2026-05-12 01:50 UTC, tag `v0.2.1.8` commit `b7b5c71`). **v0.2.1.8 entregou:** **#205 NOVO P0 single source refresh token** (storm xx:00 fix — `DoseSyncWorker.java` + `DosyMessagingService.java` removem chamadas paralelas `/auth/v1/token`; consomem `access_token` cached SharedPref atualizado pelo `useAuth.jsx` em SIGNED_IN/TOKEN_REFRESHED/INITIAL_SESSION via plugin `updateAccessToken(accessToken, accessTokenExp)`. JS supabase-js = ÚNICA fonte refresh. Antes: SQL revelou 20+ refreshes/min em xx:00 mesma session, lifespan 16min, user re-login 9-12h cycle). Plus #204 expand fixes A1/A2/B/C + optimistic CRUD completos (updatePatient/updateTreatment/pause/resume/end Treatment/registerSos) + forms PatientForm + TreatmentForm edit path offline + `useOfflineGuard` hook + `OfflineNotice` component pra features FORA queue (LGPD export/delete, shares, SOS rules, templates) com bloqueio + toast "Sem conexão — requer internet". 22 checks device-only S25 Ultra em [`Validar.md`](Validar.md). **Master @ v0.2.1.7 release fechada:** **#204 Mutation queue offline** (TanStack `networkMode: 'offlineFirst'` + `setMutationDefaults` por chave 12 mutations + bridge Capacitor.Network ↔ onlineManager + persist mutations + resumePausedMutations + OfflineBanner PT-BR) e **#207 Defesa em profundidade alarme crítico** (5 fixes: advanceMins fallback `?? 0`; SCHEDULE_WINDOW_MS 48h→168h + DoseSyncWorker HORIZON_HOURS 72→168; drop diff-and-apply idempotência; REQUEST_IGNORE_BATTERY_OPTIMIZATIONS manifest + plugin methods + UX onboarding; Sentry breadcrumbs). Plus reestruturação `contexto/` V2: README entry point bulletproof Passos 0-14 + novo `Validar.md` rastreio validações manuais entre sessões + memory project-scoped reorganizada. 10 checks device-only S25 Ultra em [`Validar.md`](Validar.md) (acumular). Antes: 10 itens fechados v0.2.1.5/v0.2.1.6: #195+#196 logout cascade fix (SIGNED_OUT spurious detection + push_sub preservada) + #197 cron notify-doses 1min fallback + #198 install/upgrade detection + skip scheduleDoses durante loading + #199 cleanup push_subs stale > 30d + #200+#200.1 HORIZON cron 30h + rescheduleAll idempotente + doc shadows + #201 telemetria auth events PT-BR + painel admin /auth-log + #202 mutex/debounce useAppResume previne refresh storm + #203 som alarme `dosy_alarm.mp3` 96kbps mono. Closed Testing track ATIVO desde 2026-05-06 (#130 #158 resolvidos).
