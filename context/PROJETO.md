...
| Campo | Valor |
|---|---|
| Versão em curso | `v0.2.8.1` (branch `0.2.8.1`, vc 103) — hotfix QA/lint/grant `medications_catalog` + drain offline rollback semântico (Gemini close) |
| Último SHIPPED Play Console | `v0.2.8.0` (vc 102, 2026-05-25 01:48 BRT) — **MutationDrainWorker nativo Java** (B102 FECHADO categoricamente: Worker WorkManager 15min CONNECTED drena `pending_mutations` SharedPreferences independente da WebView/JS suspensos em Doze; refresh nativo Java) |
| Anterior em master | `v0.2.7.0` (vc 101, consolidado em vc 102 no Play) — Refactor Sync v2: sessionManager + Zustand + markDose + RPCs v3 idempotentes (`mutation_log` PK) + 5 commits hardening |
| Tag atual master | `v0.2.8.0` (`e8d776f`) — `v0.2.7.0` (`a80a349`) também tagged |
| Branch ativa | `0.2.8.1` (commit `672dc2b`) — aguardando merge master + decisão close release |
| Vercel prod | `dosymed.app` v0.2.8.0 atual (auto-deploy a partir de master) |
| B102 ("dose marcada não persiste após fechar app") | ✅ **CLOSED v0.2.8.0** — cadeia 3 root causes: (1) `mutateAsync` sem await em DoseModal — fix v0.2.6.12 · (2) `processLock` órfão supabase-js pós-Doze — fix sessionManager v0.2.7.0 · (3) JS suspended em Doze deep — fix MutationDrainWorker v0.2.8.0 |

**Detalhe v0.2.8.1 (Fase 1 Plano Gemini)**: `vitest.config.js` exclui `e2e/**`; `dateUtils.test.js` espera 0h em vez de 6h pro mock `rangeNow('24h')`; `statusUtils.test.js` cobre 5 statuses incluindo `cancelled` traduzido; `markDose.js._runDrain` agora chama `revertDose` + `emitMutationError` em erros lógicos não-409 (401/403/404 antes silentes); `TreatmentForm.jsx` move `durationUnit` reativo do `useEffect` pra handlers síncronos (elimina warning `react-hooks/set-state-in-effect`); `notifications/index.js` remove `setPermState` duplicado; migration `20260525124500_v0_2_8_2_grant_medications_catalog_select.sql` aplicada em prod (4 grantees) fix HTTP 403 silencioso no autocomplete `search_medications` RPC. 66 testes vitest passando, 0 erros ESLint, build + cap sync + assembleDebug OK, QA Appium emulador: autocomplete grant fix ✅ + form switch ✅. Pendente device físico (Validar.md) + close release decision.

**Detalhe v0.2.8.0 (commits `4d212ee` + `9ae8414` + `54c7e32` + merge `e8d776f`)**: `pendingMutationsQueue.js` migra `idb-keyval`→`@capacitor/preferences` (SharedPreferences `CapacitorStorage` key `dosy_pending_mutations`); migration one-way IDB→Prefs no boot `main.jsx`; `markDose.js` retry 3× erro real antes de descartar; `com.dosyapp.dosy.sync.MutationDrainWorker.java` + `MutationQueueStore.java` Worker WorkManager periódico 15min `NetworkType.CONNECTED` policy KEEP via `MainActivity.enqueueMutationDrainWorker`; refresh nativo Java (Opção B) POST `/auth/v1/token?grant_type=refresh_token` persiste tokens em `dosy_sync_credentials`; SEM `setRequiresBatteryNotLow` (healthcare > bateria); ProGuard `-keep class com.dosyapp.dosy.sync.**`. Estimativa egress: 0 bytes idle, ~50KB cenário patológico, ~100KB pior caso. **Menos agressivo que watchdog 30s do v0.2.7.0**.

**Detalhe v0.2.7.0 (commits `7d47726`+`69e1879`+`c1ce900`+`2f9a069`+`a80a349`)**: Refactor Sync v2 4 fases — Foundation (sessionManager + authedRpc + Promise.race timeout + AuthLostError + mutex refreshPromise) → Zustand 3 stores (doseStore + patientStore + treatmentStore) remove TanStack persist → markDose pipeline com queue antes RPC + timeout dinâmico cold-start 30s + RPCs v3 idempotentes (`mutation_log` PK request_id) + scheduleRetryDrain backoff 5s→60s self-perpetuating → Realtime simplificado postgres_changes → refetch debounced 1.5s. Hardening pós-QA: cold-start dynamic timeout + retry backoff + AppHeader overdueCount lê Zustand reativo + bridge Zustand→TanStack invalidate + AuthLost agenda retry + watchdog 30s queue stuck. Resolve 90% B102.
...
**Deploy web (Vercel):** https://dosymed.app (custom domain, auto-deploy via Vercel CLI)
**Deploy Android (Play Store):** **Internal Testing ativo** — `com.dosyapp.dosy` versionCode 102 / versionName 0.2.8.0 publicado 2026-05-25 01:48 BRT (MutationDrainWorker nativo + Refactor Sync v2 consolidados). Estado atual sempre em [`context/STATE.md`](STATE.md). Closed Testing externo via Google Group #129-#133 pendente. Histórico completo de releases (v0.1.7.x → v0.2.8.x) em [`context/ROADMAP.md §6.3`](ROADMAP.md) + [`context/updates/`](updates/).
...
│   ├── build.gradle            # versionCode 103 / versionName 0.2.8.1 (atualiza a cada release)
