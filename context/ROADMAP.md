...
**release/v0.2.3.15 EM CURSO** (vc 78, refactor Fase 1) — RealtimeGate + versioned cache + debounces alinhados + MultiDoseModal per-dose busy + Dashboard refresh barrier. Validado em emulador (Pixel 8 teste-plus + Pixel 9 Pro teste-free): 3 marcações em sequência sem flicker, status persiste 25s, cross-device sync OK. Pendente: Passo 10.5 STOP pré-AAB + upload Play Console + INSERT `app_releases`. Refactor_Full.md (raiz) define plano completo de 5 fases (Fases 2-5 vêm depois). Próximos passos pós-ship v0.2.3.15:

- **#006** Device validation 3 devices físicos (FASE 17 manual user) — inclui validação device v0.2.3.13 disclaimer paciente compartilhado (#300 em STATE.md)
...
- **Refactor Fase 2** (consolidação de alarmes + plugin Java fixes) — `Refactor_Full.md §5 Fase 2` (4 sem.)

### Releases recentes (top 4 — histórico completo em §6.3 + `context/updates/`)

- **v0.2.3.15** (em curso, vc 78) — Refactor Fase 1: RealtimeGate per-queryKey TTL 2.5s + versioned cache `_localActedAt` + Realtime debounce 1s→2.5s + mutation refetch 2s→1.5s. MultiDoseModal `pendingDoseId` per-dose elimina disabled coletivo que travava fila. Dashboard handleRefresh aguarda mutations doses drenarem (≤2s) antes do pull-to-refresh. Mata RC-1 "status volta do nada" e "botão preso" reportados em v0.2.3.14. Plano completo de 5 fases em `Refactor_Full.md` (raiz).
- **v0.2.3.14** (2026-05-19, vc 77) — 3 fixes P2 user-reported: #0010 banner mostra version_name correto (DB autoritativa primeiro, Play Core só se shape semver) + #0011 modal mandatory robusto a race `currentVersionCode=null` (query upper-bound-only) + #0009 share error UI com retry quando 401 JWT expira (em vez de "Carregando..." infinito). Empilhamento C: Sentry breadcrumbs + copy fallback sanitization + debug toggle `__dosyForceFallback` → [updates/](updates/2026-05-19-release-v0.2.3.14.md)
- **v0.2.3.13** (2026-05-18, vc 76) — Plano A primeira etapa: AlarmReceiver pre-check HTTP + cache SharedPrefs fallback + disclaimer destacado pra paciente compartilhado quando offline/erro server. Edge envia `isShared` por dose. Zero regressão pra paciente não shared. 5/6 cenários E2E validados em emulator + Appium
...
- **Δ v0.2.6.1** (em curso, vc 85) — **3 ações A→B restantes do audit dosy-app/docs + UI TTL SharePatientSheet.** Escopo: (1) alert level per-treatment per-user (Crítico/Push/Silencioso) — schema nova tabela `treatment_alert_prefs` + toggle inline no card; (2) conflict 409 prompt explícito em mutations dose (mutationRegistry onError → ConfirmDialog); (3) PostHog instrumentação 12 eventos de categoria; (4) UI SharePatientSheet com radio Permanente/Temporário + date picker; (5) Edge Function `share-expiry-cron` rodando 1×/h. Branch `release/v0.2.6.1` aberta 2026-05-23.
