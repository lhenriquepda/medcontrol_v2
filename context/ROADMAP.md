...
- **Δ v0.2.8.1** (EM CURSO 2026-05-25, vc 103) — **Correções e QA v0.2.8.1 (Fase 1)**. Correção de bugs críticos e warnings do linter: (1) `vitest.config.js` exclui pasta `e2e/**` do runner do Vitest; (2) `dateUtils.test.js` corrige expectativa de `rangeNow('24h')` para 0h (meia-noite local); (3) `statusUtils.test.js` atualiza contagem para 5 status incluindo `cancelled` com tradução; (4) `markDose.js` corrige o drain offline (`_runDrain`) para reverter o estado local e exibir toast via `emitMutationError` quando ocorrem erros lógicos (ex: 401, 403, 404) no Supabase; (5) `notifications/index.js` remove chamada redundante a `setPermState` no montador; (6) `TreatmentForm.jsx` substitui `useEffect` reativo que atualizava `durationUnit` por atualizações imperativas e síncronas nos eventos de mudança de chip de modo e horas de intervalo, limpando warnings de renders em cascata.
- **Δ v0.2.8.1** (EM CURSO 2026-05-25, branch `0.2.8.1` vc 103 não-mandatory — pre-merge master + sem AAB Play Console) — **QA + lint + grant medications_catalog (Plano Gemini Fase 1)**. Commits `3956d38` (Gemini) + `672dc2b` (sync docs + migration). Fixes: vitest exclude `e2e/**` + asserções corrigidas `dateUtils.test.js` (rangeNow 24h `0h` em vez de `6h`) e `statusUtils.test.js` (5 statuses incluindo `cancelled` traduzido); `markDose.js._runDrain` `revertDose` + `emitMutationError` em erros lógicos não-409 (401/403/404 antes silentes); `TreatmentForm.jsx` `durationUnit` reativo via `useEffect` → handlers síncronos (mata warning `react-hooks/set-state-in-effect`); `notifications/index.js` remove `setPermState` duplicado; migration `20260525124500_v0_2_8_2_grant_medications_catalog_select.sql` aplicada em prod (GRANT SELECT pra anon/authenticated/service_role) — fix HTTP 403 silencioso no autocomplete `search_medications` RPC. Validação QA emulador: 66 testes vitest verde, 0 erros ESLint, build + cap sync + assembleDebug OK, Appium autocomplete sugestões aparecem sem 403 + form switch sem render loop. Bug latente descoberto (não regressão): badges CMED/DCB não renderizam por dedup local-vs-catálogo em `MedNameInput.jsx:142-144` (item próxima release). Aguarda merge master + AAB + Play Console + close. → [docs/Gemini/implementation_plan.md](../docs/Gemini/implementation_plan.md)
- **Δ v0.2.8.0** (SHIPPED 2026-05-25, vc 102 não-mandatory) — **MutationDrainWorker nativo Android — B102 FECHADO CATEGORICAMENTE**. Resolve último 10% caminho B102: marca dose com WebView suspended em Doze deep → JS timers param → queue stuck até user reabrir (limite arquitetural JS em WebView). Solução: Worker Java independente do WebView. **Passo 1** (commit `4d212ee`): pendingMutationsQueue migra de idb-keyval (IDB JS-only) pra @capacitor/preferences (SharedPreferences acessível JS+Java), key `dosy_pending_mutations` group `CapacitorStorage`. Migration one-way no boot main.jsx (decisão user #5). markDose 3× retry erro real antes descartar (decisão user #3). **Passo 2+3+4** (commit `9ae8414`): `com.dosyapp.dosy.sync.MutationDrainWorker` + `MutationQueueStore` Java. PeriodicWorkRequest 15min CONNECTED policy KEEP via `MainActivity.enqueueMutationDrainWorker`. POST RPCs v3 via HttpURLConnection. **Refresh nativo Java Opção B (decisão user #2)** — POST `/auth/v1/token?grant_type=refresh_token`, persiste tokens em `dosy_sync_credentials` SharedPreferences (~200 linhas extras). SEM `setRequiresBatteryNotLow` (decisão #4). Test plan §6 expandido com 5 cenários adb + logcat + Supabase SQL. ProGuard `-keep class …sync.**`. **Passo 5** (commit `54c7e32`): Refactor_Sync_v2 §5.5 drain agora em 4 momentos (boot/resume/online/Worker 15min). BUGS.md entry v0.2.8.0 com B102 FECHADO + explicação cadeia 3 root causes. Estimativa egress: 0 bytes idle, ~50KB cenário patológico, ~100KB pior caso. Menos agressivo que watchdog 30s do v0.2.7.0. Validação QA emulador parcial OK: Worker enqueued + primeiro cycle quick-exit "queue vazia". Build assembleDebug + bundleRelease OK. → [Plano_Worker_Native_v028.md](../Plano_Worker_Native_v028.md)
- **Δ v0.2.7.0** (SHIPPED 2026-05-25, vc 101 não-mandatory) — **Refactor Sync v2 — 4 fases + hardening (resolve 90% B102)**. **Fase 1** sessionManager + authedRpc com Promise.race timeout + AuthLostError class + onAuthLost listener (mutex refreshPromise pra dedupar refresh concorrentes). **Fase 2** Zustand 3 stores (doseStore + patientStore + treatmentStore) com Map + revert snapshot + remove TanStack persist (PersistQueryClientProvider → QueryClientProvider direto). **Fase 3** markDose pipeline (optimistic patch + queue add antes RPC + authedRpc com timeout dinâmico 30s cold-start) + pendingMutationsQueue IDB sobrevive kill + RPCs v3 idempotentes (mutation_log PK request_id) + drainPendingMutations FIFO serial + scheduleRetryDrain backoff 5s→60s self-perpetuating. **Fase 4** Realtime simplificado postgres_changes → scheduleRefetch debounced 1.5s → fetchDashboard (remove gate/versionedCache). **Hardening pós-QA real** (5 commits `7d47726` `69e1879` `c1ce900` `2f9a069` `a80a349`): timeout dinâmico cold-start + retry persistente backoff + AppHeader overdueCount lê Zustand reativo + bridge Zustand→TanStack invalidate (DoseHistory/Reports/Analytics) + AuthLost também agenda retry + watchdog 30s queue stuck. Resolve B102 em 90% casos (kill mid-RPC + boot drain idempotente). **Remaining 10%** (suspended Doze) coberto por v0.2.8.0 Worker nativo. → [Refactor_Sync_v2.md](../Refactor_Sync_v2.md)
- **Δ v0.2.6.6** (em curso 2026-05-23, vc 91 não-mandatory) — **Bug crônico #0023 "perde comunicação BD após idle" RESOLVIDO**. Diagnóstico via 4 agentes paralelos (Supabase client + mutation flow + native lifecycle + DB schema) — todos convergiram: JWT expira em idle 1h+ + WebView pausada Doze + processLock supabase-js órfão mid-refresh + RPC v2 retorna {ok:false, code:401} HTTP 200 (não throw) + onError fazia rollback silent + Sentry sample 1% dilui. **8 fixes:** F1 onlineManager re-sync pós-resume (Capacitor Network) + F2 resumePausedMutations pós-soft-recover + F3 watchdog ping timeout 5s (token zombie → signOut+reload) + F4 rpcV2WithAuthRetry wrapper (refresh+retry em 401) + F5 mutationErrorBus + MutationErrorListener (toast UI em 8 handlers) + F7 DROP 8 RPC overloads stale (zero ambiguidade PostgREST) + F8 Sentry visibilidade total 401/403/409. Postgres_log prod confirmou `permission denied for table doses/patients` recorrente. → [updates/](updates/2026-05-23-release-v0.2.6.6.md)
- **Δ v0.2.6.5** (SHIPPED 2026-05-23 16:40 BRT, vc 90 não-mandatory) — **UX mobile + AdMob banner display + bug #0020 dose state stale**. #0018 Keyboard-aware scroll (hook global focusin + Capacitor Keyboard.keyboardWillShow → scrollIntoView offset dinâmico, funciona em todos forms). #0019 AdMob banner duplo fix: (a) initializeForTesting amarrado ao test ad unit (não !PROD que vite build sempre seta true → NO_FILL); (b) MainActivity.java mede WindowInsets statusBars + displayCutout nativo, injeta `--system-status-bar-height` CSS var per-device. #0020 confirmDose/skipDose/undoDose/registerSos onMutate cancela AMBAS queries (`['doses']` + `['dashboard-payload']`) — antes só doses → dashboard-payload in-flight sobrescrevia patch optimistic. PullToRefreshOverlay reutilizável (top respeita --ad-banner-height) + PTR adicionado em Dashboard/DoseHistory/Patients/PatientDetail/Analytics. LiveReload dev infra (DOSY_LIVERELOAD=1 opt-in). → [updates/](updates/2026-05-23-release-v0.2.6.5.md)
- **Δ v0.2.6.4** (SHIPPED 2026-05-23 11:27 BRT, vc 89 mandatory) — **Roteiro Sprint P0+P3+P9 (foundation invisível + UX bulk categorize + cleanup catálogo + regex tightening)**. P3.2 audit_log LGPD append-only (table 19 actions + retention cron mensal). P3.3 feature_flags master switch runtime (6 seed flags + rollback <5min sem deploy). P9.6 BulkCategorizeModal Analytics donut drill-down (tap "Não classificado" → modal lista meds NULL + sugestão IA + bulk apply cascata treatments + doses). P9.7 v_categorization_health view + admin RPC. P0.2 regex pina tightened (anti_hipertensivo PRIMEIRO + gender BR (a|o)? + \b boundary). P0.5 cleanup 458 → 362 catalog rows (96 movidas: 82 antibiotico/65 anti_hipertensivo/64 hormonal/47 GI/42 antidepressivo/18 anticoagulante/etc). P0.1 SQL versioning replay files v0.2.6.4 (3 migrations + cleanup). Build verde 17.48s lint 100/100 max. → [updates/](updates/2026-05-23-release-v0.2.6.4.md)
- **Δ v0.2.6.3** (SHIPPED 2026-05-23 10:29 BRT, vc 88 mandatory) — **HOTFIX #2 categorização inteligente + origem badge**. 3 bugs P0 reportados v0.2.6.2 vc 87: (#0015) Histórico ainda em "Outro" → cache IDB stale `buster v1`→`v2` force purge único. (#0016) Autofill sticky "sempre Antidepressivo" → useEffect re-aplica classifyResult em TODA mudança `[classifyResult, medName]` + distingue manual pick de autofill + LIMPA stale autofill. (#0017) RPC `classify_medication_robust` NÃO EXISTIA → migration cria 5-tier server-side (DCB exact 1.0 / catalog exact 0.95 / catalog LIKE 0.85 / principio LIKE 0.75 / heurística sufixo 0.55 word-boundary-aware). MedNameInput dropdown agora exibe 3 badges origem: DCB ANVISA (azul) / CMED (verde) / SEU (laranja peach). QA Android Pixel8 vc 88 20/20 PASS. → [updates/](updates/2026-05-23-release-v0.2.6.3.md)
